defmodule Craftplan.Catalog.Product.Calculations.UnitCost do
  ...
  def calculate(records, _opts, _arguments) do
    Enum.map(records, &unit_cost/1)
  end

  defp unit_cost(%{active_bom: %NotLoaded{}} = record), do: unit_cost_from_recipe(record)
  defp unit_cost(%{active_bom: nil} = record), do: unit_cost_from_recipe(record)

  defp unit_cost(%{active_bom: bom}) do
  ...
  defp unit_cost_from_recipe(%{recipe: %NotLoaded{}}), do: D.new(0)
  defp unit_cost_from_recipe(%{recipe: nil}), do: D.new(0)

  defp unit_cost_from_recipe(%{recipe: recipe}) do
