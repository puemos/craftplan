defmodule CraftplanWeb.ImportModalComponent do
  ...
  def update(assigns, socket) do
    ...
    socket =
      if socket.assigns[:_upload_init] do
        ...
        allow_upload(socket, :csv, accept: [".csv", "text/csv"], max_entries: 1)
    ...
    <div id={@id <> "-wrap"}>
      <.modal :if={@show} id={@id} title={"Import " <> String.capitalize(@entity || "")} show={true}>
        <div class="h-[600px] overflow-auto">
          <div phx-target={@myself} class="bg-white/95 sticky top-0 z-20 -mx-6 mb-4 px-6 py-3 backdrop-blur supports-[backdrop-filter]:bg-white/60">
            <.stepper steps={["Provide CSV", "Mapping", "Import"]} current={wizard_label(@wizard_step)} goto_event="wizard_goto" />
          </div>
          ...
          <.form :if={@wizard_step == :provide} for={@csv_form} id="csv-select-form" phx-target={@myself} phx-submit="csv_import">
          ...
          <div :if={@wizard_step in [:map, :import]} class="mt-6">
            <div class="mb-2 flex items-center justify-between">
              ...
              <div :if={@csv_errors && @csv_errors != []} class="text-xs text-red-700">{length(@csv_errors)} error(s)</div>
            </div>
            <div class="mb-2">
              <div role="tablist" aria-orientation="horizontal" class="bg-stone-200/50 inline-flex h-9 rounded-lg p-1">
                <button type="button" phx-target={@myself} phx-click="map_set_tab" phx-value-tab="mapping" class={["inline-flex items-center justify-center whitespace-nowrap rounded-md border px-3 py-1 text-sm font-medium", @map_view_tab == :mapping && "border-stone-300 bg-stone-50 shadow", @map_view_tab != :mapping && "border-transparent"]}>Mapping</button>
                <button type="button" phx-target={@myself} phx-click="map_set_tab" phx-value-tab="preview" class={["inline-flex items-center justify-center whitespace-nowrap rounded-md border px-3 py-1 text-sm font-medium", @map_view_tab == :preview && "border-stone-300 bg-stone-50 shadow", @map_view_tab != :preview && "border-transparent"]}>Preview</button>
                <button type="button" phx-target={@myself} phx-click="map_set_tab" phx-value-tab="errors" class={["inline-flex items-center justify-center whitespace-nowrap rounded-md border px-3 py-1 text-sm font-medium", @map_view_tab == :errors && "border-stone-300 bg-stone-50 shadow", @map_view_tab != :errors && "border-transparent"]}>Errors</button>
              </div>
            </div>
            <div class="rounded-md border bg-white">
              <div :if={@dry_run_summary && @map_view_tab in [:preview, :errors]} class="border-b px-3 py-2 text-xs text-stone-700">{@dry_run_summary}</div>
              <div class="max-h-96 overflow-auto p-2">
                <div :if={@map_view_tab == :mapping}>
                  <.form :if={@csv_headers != [] and @wizard_step in [:map, :import]} for={to_form(@csv_mapping)} id="csv-mapping-form" phx-target={@myself} phx-submit="csv_validate">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <%= for f <- @config.fields do %>
                        <.input type="select" name={"mapping[#{f.name}]"} label={f.label} options={if f.required, do: @csv_headers, else: ["" | @csv_headers]} value={@csv_mapping[f.name]} />
                      <% end %>
                    </div>
                  </.form>
                </div>
                ...
                <div :if={@map_view_tab == :errors}>
                  ...
                  <div :if={@csv_errors && @csv_errors != []}>
                    <table class="min-w-full divide-y divide-red-200 border">
                      <thead class="bg-red-50">
                        <tr>
                          ...
                          <th class="px-2 py-1 text-left text-xs font-medium text-red-700">Message</th>
                        </tr>
                      </thead>
                    ...
                    <div :if={length(@csv_errors) > 25} class="mt-1 text-xs text-stone-600">Showing first 25 errors of {length(@csv_errors)}.</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <:footer>
          <div class="flex items-center gap-2">
            <.button :if={@wizard_step == :provide} type="submit" id={@id <> "-next"} form="csv-select-form">Next</.button>
            <.button :if={@wizard_step == :map and @map_view_tab == :mapping} type="submit" id={@id <> "-validate"} form="csv-mapping-form">Verify</.button>
            <.button :if={@wizard_step == :map} id={@id <> "-next-import"} type="button" phx-target={@myself} phx-click="csv_import_final" disabled={@csv_errors && @csv_errors != []}>Next</.button>
            <.button :if={@wizard_step == :import} id={@id <> "-run-import"} type="button" phx-target={@myself} phx-click="csv_run_import" variant={:primary}>Import</.button>
            <.button variant={:outline} type="button" phx-target={@myself} phx-click="wizard_close">Close</.button>
          </div>
        </:footer>
      </.modal>
    </div>
  ...
  defp do_csv_dry_run(entity, csv, delimiter, mapping, socket) do
    ...
    cond do
      importer && function_exported?(importer, :dry_run, 2) ->
        case importer.dry_run(csv, delimiter: delimiter, mapping: mapping) do
          {:ok, %{rows: rows, errors: errors}} ->
            ...
            {:noreply,
             ...
             |> assign(:dry_run_summary, msg)
             |> assign(:csv_errors, errors)
             |> assign(:map_view_tab, if(errors != [], do: :errors, else: :preview))
             |> assign(:wizard_step, :map)}
        end
